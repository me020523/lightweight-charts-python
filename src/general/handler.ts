...
export class Handler {
    ...
    getPoint(logical: Logical) {
        if (!logical) {
            return null;
        }
        if (this.series.data().length != 0) {
            return this.series.dataByIndex(logical);
        }
        if (this._seriesList.length == 0) {
            return null;
        }

        return this._seriesList[0].dataByIndex(logical);
    }
    ...
    public static syncCharts(
        ...
        const setParentCrosshair = (param: MouseEventParams) => {
            ...
            crosshairHandler(parentChart, parentChart.getPoint(param.logical!));
        };
        const setChildCrosshair = (param: MouseEventParams) => {
            //crosshairHandler(childChart, getPoint(parentChart.series, param));
            crosshairHandler(childChart, childChart.getPoint(param.logical!));
        };
            ...
            if (searchWindow.style.display === "none") {
                ...
                if (event.key === "Enter")
                    window.callbackFunction(
                        `search${chart.id}_~_${sBox.value}`,
                    );
                searchWindow.style.display = "none";
                sBox.value = "";
                return true;
